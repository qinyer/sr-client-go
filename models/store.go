...
type Store struct {
	...
	DaojiaOperationStatus int           `json:"daojia_operation_status"` // 暂缺
	PhoneNumbers          []string      `json:"phone_numbers"`           // 店仓电话列表，格式如：075588888888
...
type LocationInfo struct {
	CountryCode  string     `json:"country_code"`  // 国家编码，CN 字段长度最小 1 字节，长度最大 64 字节
	CountryName  string     `json:"country_name"`  // 国家名称，中国 字段长度最小 1 字节，长度最大 64 字节
	ProvinceCode int        `json:"province_code"` // 省份编码，使用《统计用区划代码和城乡分代码编制规则》，440000
	...
	GeoInfo      *[]GeoInfo `json:"geo_info"`      // 坐标信息
}

// BasicProps
type BasicProps struct {
	Name          string           `json:"name"`           // 店仓名称建议格式：品牌名称（XX店） 字段长度最小 1 字节，长度最大 32 字节
	OpeningTime   string           `json:"opening_time"`   // 店仓开业时间 时间戳 毫秒 字段长度为长度为 13 字节
	OperatingTime *[]OperatingTime `json:"operating_time"` // 为空默认周一至周日，10:00-22:00
}

// DeliveryInfo
type DeliveryInfo struct {
	RangeType int         `json:"range_type"` // 配送范围类型 1：圆形；3：多边形；store支持到家配送时必填
	Radius    string      `json:"radius"`     // 配送半径 range_type=1时，必填 字段长度最小 1 字节，长度最大 128 字节
	GeoGroup  *[]GeoGroup `json:"geo_group"`  // 坐标信息组合，坐标点的二维数组，表示多个形状 range_type=3时，必填
}

// GeoInfo
type GeoInfo struct {
	Latitude  float64 `json:"latitude"`  // 店仓纬度，如：22.5200
	Longitude float64 `json:"longitude"` // 店仓经度，如：113.9400
	Type      int     `json:"type"`      // 填写了longitude和latitude则必填 1：腾讯/高德地图；2：百度地图：3：国际标准
}

// OperatingTime
type OperatingTime struct {
	DateZone string `json:"date_zone"` // 日期区间，例：周一至周日；字段长度最小 1 字节，长度最大 64 字节
	TimeZone string `json:"time_zone"` // 时间区间，例：10:00-22:00；字段长度最小 1 字节，长度最大 64 字节
}

// GeoGroup
type GeoGroup struct {
	Geos *[]GeoInfo `json:"geos"` // 坐标点信息
}
