...
type Sku struct {
	...
	ExternalSpuId       string          `json:"external_spu_id"`       // 您为商品SPU分配的唯一ID。 色码款商品必填。 字段长度最小 1 字节，长度最大 128 字节
	SkuBarcode          string          `json:"sku_barcode"`           // 商品条形码，超市商品必填。字段长度最小 1 字节，长度最大 64 字节
	ImgUrls             *[]ImgUrl       `json:"img_urls"`              // 商品图片url列表 数组最大长度 10
	ProductProps        *ProductProps   `json:"product_props"`         // 商品属性
	CategoryProps       *[]CategoryProp `json:"category_props"`        // 类目属性
...
type ImgUrl struct {
	...
	DetailImgs  *[]Img `json:"detail_imgs"`  // 详情页图片信息列表 数组最大长度 10
}

//CategoryProp
type CategoryProp struct {
	ExternalCategoryIdLeaf string `json:"external_category_id_leaf"` // 您为商品分配的叶子类目 id。字段长度最小 1 字节，长度最大 255 字节
	CategoryType           int    `json:"category_type"`             // 固定传 2，表示后台类目
}
...
type ProductProps struct {
	Color *Color `json:"color"`  // 商品颜色属性，色码款商品必传
	IsO2o bool   `json:"is_o2o"` // true：线上线下同款；false：线上线下不同款
	Size  string `json:"size"`   // 商品尺码，色码款商品必传 字段长度最小 1 字节，长度最大 8 字节
}

// Color
type Color struct {
	ColorRgb  string `json:"color_rgb"`  // 颜色RGB编码 字段长度最小 1 字节，长度最大 64 字节
	ColorName string `json:"color_name"` // 颜色名称，如黄色 字段长度最小 1 字节，长度最大 64 字节
}
